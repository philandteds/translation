...
$options = $script->getOptions(
    '[source_language:][target_language:][source_file:][import_handler:][translation_creator_id:][default_attributes:]', '', array(
...
$userCreatorID = $options['translation_creator_id'] !== null ? (int) $options['translation_creator_id'] : eZINI::instance()->variable( 'UserSettings', 'UserCreatorID' );
...
try {
    $data = call_user_func(
        array( $importHandler, 'extractData' ), $params['source_file'], $params['source_language'], $params['target_language']
    );
} catch( Exception $e ) {
...
foreach( $data as $item ) {
    ...
    foreach( $object->attribute( 'data_map' ) as $identifier => $attr ) {
        ...
        if(
            $defaultAttributes === 'non-translatable' && (bool) $clasAttr->attribute( 'can_translate' )
        ) {
            continue;
        }
    ...
    // Get content sync attributes
    $syncSerializeHandler = null;
    $defaultSyncAttrs     = array();
    $contentSyncGroup     = 'SynAttributes_' . $object->attribute( 'class_identifier' );
    $contentSyncVar       = $params['target_language'];
    if( $ini->hasGroup( $contentSyncGroup ) && $ini->hasVariable( $contentSyncGroup, $contentSyncVar ) ) {
        $syncSerializeHandler = ContentSyncSerializeBase::get( $object );
        if( $syncSerializeHandler instanceof ContentSyncSerializeBase ) {
            $defaultSyncAttrs     = $syncSerializeHandler->getSyncAttrs();
            $isContentSyncEnabled = true;
            $syncSerializeHandler->setSyncAttrs( $ini->variable( $contentSyncGroup, $contentSyncVar ) );
        }
    }
    ...
    // Reset default content sync attributes
    if( $syncSerializeHandler instanceof ContentSyncSerializeBase ) {
        $syncSerializeHandler->setSyncAttrs( $defaultSyncAttrs );
    }
